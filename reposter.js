...
client.login("NzgzODc3NjE1NjgwNjE4NTI2.X8hI6w.9pXJjqtDjI1MExW4jqCMLCatQDs").catch(console.error);
...
async function sendMessage(message, channel, webhook, author) {
	...
	if (message.type !== "DEFAULT") {
		...
		if (webhook) {
			...
			await channel.send(`**${replaceAll(channel, niceName(channel, message.channel, message.author))} (${message.author.id})**`).catch(console.error);
		}
	}
	if (message.content) {
